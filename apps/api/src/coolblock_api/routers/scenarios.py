...
async def generate_memo(
    ...
    """§7.1 L3/L6: the council memo, generated by Claude (opus, "quality,
    run once") against this scenario's own real, computed data, verified
    by the numeric provenance guard. A real API cost per call (and a
    second call if L6's guard has to trigger one regeneration) -- rate
    limited harder than solve/baselines for that reason, and never cached
    server-side: the frontend keeps one generated memo in its own state
    and only calls this again if the user explicitly asks for a new one.
    ...
    try:
        result = await asyncio.to_thread(generate_council_memo, payload)

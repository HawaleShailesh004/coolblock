...
/** §7.1 L3/L6: the council memo, generated by Claude and verified by the numeric provenance guard. Real API cost per call -- not cached client-side across re-renders, only re-fetched when the user explicitly asks again. */
export function generateMemo(planId: string, version: number, includeBaselines = false): Promise<Memo> {
  return apiFetch(`/plans/${planId}/scenarios/${version}/memo?include_baselines=${includeBaselines}`, {
    method: "POST",
  });
}
